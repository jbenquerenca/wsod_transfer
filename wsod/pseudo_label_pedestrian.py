...
folder, eurocity_train, eurocity_val, tag, it, th = sys.argv[1:]
print('pseudo label params:', folder, eurocity_train, eurocity_val, tag, it, th)
...
annos = []
id = 0
for img_id, (t, p) in tqdm(p_train.items(), mininterval=20):
...
fn = f'datasets/{target_folder}/%s' % json_target
print ("save to " + fn + '\n')
with open(fn,'w') as f:
    json.dump(d, f)
...
dataset_list = (eurocity_train, eurocity_val)
transforms = None
datasets = build_dataset(dataset_list, transforms, DatasetCatalog, False)
...
for split, d, p in zip(['train','val'], [d1, d2], [p_train, p_val]):
    ...
    json_coco = 'eurocity_%s_%s_it%s_%s.json' % (split, tag, it, th)
    fn = 'datasets/EuroCity/annotations/%s' % json_coco
    print ("save to " + fn + '\n')
    with open(fn,'w') as f:
        json.dump(d, f)
